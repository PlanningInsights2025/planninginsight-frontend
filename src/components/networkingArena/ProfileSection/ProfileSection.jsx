  ...
  const fetchProfileData = async () => {
    try {
      ...
      if (response.success) {
        ...
        // Update userRole based on actual role from backend
        if (response.profile.role === 'premium' || response.profile.role === 'admin') {
          setUserRole(response.profile.role === 'admin' ? 'premium' : response.profile.role);
        }
      }
    } catch (error) {
