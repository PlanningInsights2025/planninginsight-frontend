  ...
  return (
    <div className="publishing-management">
      ...
      {activeTab === 'manuscripts' && (
        <>
          {/* Sub-tabs for Manuscripts and Research Papers */}
          <div className="neu-card tab-container">
            <div className="tabs">
              <button
                className={`tab-btn ${submissionType === 'manuscripts' ? 'active' : ''}`}
                onClick={() => setSubmissionType('manuscripts')}
              >
                <span className="tab-icon">📝</span>
                Manuscripts
                {stats.pending > 0 && (
              ...
              <button
                className={`tab-btn ${submissionType === 'papers' ? 'active' : ''}`}
                onClick={() => setSubmissionType('papers')}
              >
                <span className="tab-icon">📄</span>
                Research Papers
                {paperStats.pending > 0 && (
                  <span className="badge-count">{paperStats.pending}</span>
                )}
              </button>
            </div>
          </div>
          ...
          <div className="stats-grid">
            <div className="stat-card neu-card">
              ...
              <div className="stat-content">
                <div className="stat-value">{submissionType === 'manuscripts' ? (stats.total || 0) : (paperStats.total || 0)}</div>
                <div className="stat-label">Total {submissionType === 'manuscripts' ? 'Manuscripts' : 'Research Papers'}</div>
              </div>
            </div>
            <div className="stat-card neu-card">
              ...
              <div className="stat-content">
                <div className="stat-value">{submissionType === 'manuscripts' ? (stats.pending || 0) : (paperStats.pending || 0)}</div>
            ...
            <div className="stat-card neu-card">
              ...
              <div className="stat-content">
                <div className="stat-value">{submissionType === 'manuscripts' ? (stats.accepted || 0) : (paperStats.accepted || 0)}</div>
            ...
            <div className="stat-card neu-card">
              ...
              <div className="stat-content">
                <div className="stat-value">{submissionType === 'manuscripts' ? (stats.rejected || 0) : (paperStats.rejected || 0)}</div>
              ...
              {submissionType === 'manuscripts' ? (
                                ...
                                {manuscript.reviewedBy || manuscript.assignedEditor ? (
                                  <div className="editor-info">
                                    <div className="editor-name">
                                      {manuscript.reviewedBy ? 
                                        `${manuscript.reviewedBy.profile?.firstName || ''} ${manuscript.reviewedBy.profile?.lastName || manuscript.reviewedBy.email}`.trim() :
                                        `${manuscript.assignedEditor.profile?.firstName || ''} ${manuscript.assignedEditor.profile?.lastName || manuscript.assignedEditor.email}`.trim()
                                      }
                                    </div>
                                    {manuscript.editorReviewedAt && (
                                      <div className="text-muted" style={{fontSize: '0.85em'}}>
                                        {formatDate(manuscript.editorReviewedAt)}
                                      </div>
                                    )}
                                  </div>
                                ) : (
                ...
                <>
                  {researchPapers.length === 0 ? (
                    <div className="neu-card empty-state">
                      <div className="empty-icon">📄</div>
                      <h3>No Research Papers Submitted</h3>
                      <p>Waiting for researchers to submit papers</p>
                    </div>
                  ) : (
                    <div className="table-container">
                      <table className="neu-table">
                        ...
                        <tbody>
                          {researchPapers.map((paper) => (
                            <tr key={paper._id}>
                              ...
                              <td>
                                <div className="author-info">
                                  <div className="author-name">{paper.author?.name || 'N/A'}</div>
                                  <div className="author-email">{paper.author?.email || 'N/A'}</div>
                                </div>
                              </td>
                              <td>
                                <div className="requirement-info">
                                  <div>{paper.requirementId?.title || 'N/A'}</div>
                                  <div className="text-muted">{paper.requirementId?.topic || 'N/A'}</div>
                                </div>
                              </td>
                              <td>{formatDate(paper.submittedAt || paper.createdAt)}</td>
                              <td>
                                {paper.reviewedBy || paper.assignedEditor ? (
                                  <div className="editor-info">
                                    ...
                                    {paper.editorReviewedAt && (
                                      <div className="text-muted" style={{fontSize: '0.85em'}}>
                                        {formatDate(paper.editorReviewedAt)}
                                      </div>
                                    )}
                                  </div>
                                ) : (
                              ...
                              <td>
                                <span className={getStatusBadgeClass(paper.status || 'pending')}>
                                ...
                                <div className="action-buttons">
                                  ...
                                  {paper.file?.url ? (
                                    <a
                                      ...
                                      download
                                      className="neu-btn neu-btn-sm neu-btn-secondary"
                                      title="Download research paper file"
                                    >
                                      📥 Download
                                    </a>
                                  ) : (
        ...
        <div className="modal-overlay" onClick={() => setShowPaperModal(false)}>
          <div className="modal-content neu-card modal-large" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="manuscript-details">
              ...
              <div className="detail-row">
                <strong>Author:</strong> {selectedPaper.author?.name || 'N/A'}
              </div>
              <div className="detail-row">
                <strong>Email:</strong> {selectedPaper.author?.email || 'N/A'}
              </div>
              <div className="detail-row">
                <strong>Affiliation:</strong> {selectedPaper.author?.affiliation || 'N/A'}
              </div>
              <div className="detail-row">
                <strong>Requirement:</strong> {selectedPaper.requirementId?.title || 'N/A'}
              </div>
              <div className="detail-row">
                <strong>Topic:</strong> {selectedPaper.requirementId?.topic || 'N/A'}
              </div>
              <div className="detail-row">
                <strong>Submitted:</strong> {formatDate(selectedPaper.submittedAt || selectedPaper.createdAt)}
              </div>
              <div className="detail-row">
                <strong>Current Status:</strong> 
                <span className={getStatusBadgeClass(selectedPaper.status || 'pending')}>
            ...
            <form onSubmit={handleReviewPaper}>
              ...
              <div className="modal-actions">
                ...
                {selectedPaper.file?.url ? (
                  <a
                    href={selectedPaper.file.url}
                    target="_blank"
                    rel="noopener noreferrer"
                    download
                    className="neu-btn neu-btn-secondary"
                    title={`Download ${selectedPaper.file.filename || 'research paper'}`}
                  >
                    📥 Download File
                  </a>
                ) : (
